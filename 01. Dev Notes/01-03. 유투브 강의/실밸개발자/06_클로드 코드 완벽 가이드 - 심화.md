![[Pasted image 20260419140520.png]]
<aside> 📌

이 페이지는 **Claude Code 고급 자동화 & 확장 마스터** 영상의 핵심 내용을 정리한 치트시트입니다. 스킬, Sub-Agent, Hooks, 고급 팁까지 — 파워유저로 가는 길을 담았습니다.

</aside>

![[Pasted image 20260419140602.png]]
...
![[Pasted image 20260419140907.png]]
...
└── ppt-generator/            ← 스킬 폴더
    ...
    └── scripts/               ← 스크립트 (선택)
```
![[Pasted image 20260419141245.png]]
<aside> ⚡

**2단계 로딩 방식** 1단계: 스킬 이름과 description만 로드 (항상, ~50-100바이트) 2단계: 실제 호출 시 [SKILL.md](http://SKILL.md) 본문 + 참고 파일 전체 로드

평소에는 가볍게, 쓸 때만 전체를 불러오는 구조입니다. 자동 정리(컴팩션) 시 전체 내용은 정리되고 설명만 남습니다.

</aside>

![[Pasted image 20260419141336.png]]
아무리 좋은 규칙을 만들어도 이 컨텍스트 윈도우가 꽉 차버리면 AI가 앞에서 뭘했는지 잊어버리고 나쁜 품질의 코드를 작성해준다. 스킬은 이 문제를 깔끔하게 해결해준다. 필요할 때만 불러오고 쓰고 나면 알아서 치워준다.

![[Pasted image 20260419141700.png]]
...
![[Pasted image 20260419142426.png]]
...
**description 작성 팁** ✅ 좋은 예: "PPT 발표자료 자동 생성. 'PPT 만들어줘', '발표자료 작성' 요청 시 트리거." ❌ 나쁜 예: "문서를 생성하는 스킬" (너무 추상적)
...
## 🤖 2-1. Sub-Agent란?
![[Pasted image 20260419150959.png]]
...
**핵심 장점 4가지** • 병렬 처리 — 여러 작업을 동시에 실행해 전체 소요 시간 단축 • 컨텍스트 보호 — 메인 에이전트의 컨텍스트를 오염시키지 않음 • 전문화 — 각 Sub-Agent에 전문 역할 부여 • 재사용 — 한 번 만든 에이전트를 여러 워크플로우에서 활용

</aside>

<aside> ⚠️

**Do's & Don'ts** ✅ 독립적인 작업을 병렬로 분배 ✅ 명확한 역할과 범위 지정 ✅ 결과를 메인에서 통합 처리 ❌ 의존성 있는 작업을 무리하게 병렬화 ❌ 하나의 Sub-Agent에 너무 많은 역할 ❌ Sub-Agent 간 직접 통신 시도 (불가)
...
![[Pasted image 20260419160707.png]]
```
...
- 예시: Plan Review MCP 서버
    ...
    **요청 프롬프트:** "Gemini API를 사용하는 plan-review MCP 서버를 만들어줘. `review_plan` tool 하나만 있으면 돼. plan 텍스트를 받아서 Gemini에 리뷰를 요청하고 결과를 반환하게."
...
이 시리즈는 총 3편입니다. 1편: 입문 → 2편: 실전 워크플로우 → **3편: 고급 자동화 & 확장 (현재)** 스킬, Sub-Agent, Hooks를 조합하면 1인 개발자가 팀급 생산성을 낼 수 있습니다.

</aside>

이 시리즈에서 다룬 내용을 하나하나 해 본 순간 여러분은 더 이상 코드를 짜는 사람이 아니다.
AI 에어전트를 설계하고 오케스트레이션 하는 사람 즉 에이전틱 엔지니어가 되는 것이다.
스킬을 만들어서 AI의 능력을 확장하고 서브 에이전트를 띄워서 병렬로 일을 처리하고
훅으로 워크플로우를 자동화하는 것 이건 단순히 도구를 잘 쓰는 것이 아니라 개발의 패러다임 자체를 바꾸는 것이다. 1인 개발자가 팀 급 생산성을 내는 시대이다.
