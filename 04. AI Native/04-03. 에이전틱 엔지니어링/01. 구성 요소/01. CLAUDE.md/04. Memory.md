...
CLAUDE.md는 **개발자가 직접 작성**하는 지침이고, <font color="#ff0000">Auto memory는 Claude가 스스로 작성하는 학습 내용</font>입니다. 빌드 명령어, 디버깅 인사이트, 코드 스타일 취향 등 Claude가 작업하면서 발견한 패턴을 저장합니다.
...
<font color="#ff0000">서브에이전트 전용 메모리는 별도</font>입니다. `memory: project`로 설정된 에이전트는 `.claude/agent-memory/<agent-name>/`에 저장되어 팀과 공유되고, `memory: local`은 `.claude/agent-memory-local/<agent-name>/`에 저장되어 gitignore됩니다.
